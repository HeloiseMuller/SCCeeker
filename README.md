...
Tool developped for "", by Héloïse Muller and Cheryl P. Andam
...
`DB_20171117/` was downloaded directory from SCCmecFinder. It contains :
 ...
 - whole_cassette_SCCmec_database_REFERENCE_20171117.fasta: whole length cassetttes
 - whole_cassette_SCCmec_database_EXTENDED_20171117.fasta: extended version of the whole length cassettes
 - selectDB: file that described which of the above database to use. To midify only of one wants to update the database. By default, SCCeeker will use the EXTENDED database.
 - template_db: database used for the k-mer approach 

`test/` contains inputs to test SCCeeker

`SCCeeker.sh` runs the first step of SCCeeker (blast + an aptional k-mer appaoch) on one genome.
...
`cat_allX_blast.out` where X is the total number of analysed assemblied is the blast output.
`cat_allX_kmer.tsv` where X is the total number of analysed assemblied is the output of the k-mer approach if the option was activated.
...
`SCCeeker_summary_perFile.out` is a summary for each assembly. Are reported the presence of a mec gene, the number of candidate regins, the type of cassette if any region could be typed, and the same but reported the type only if it was validated by the k-mer approach
Many intermediate filed can be found in the directory detailed_outputs/
Some figures summaring the results can be found in the directory Figures/
